...
async def update_form_meta(
    ...
    return await FormService.process_event(
        db=db, form_id=form_id, user_id=user.id, event_in=event_in
    )
...
async def websocket_endpoint(
    websocket: WebSocket, form_id: uuid.UUID, db: AsyncSession = Depends(get_db)
):
    try:
    ...
    await websocket.send_json(
        {"type": "PRESENCE_SNAPSHOT", "users": manager.list_presence(str_form_id)}
    )
    ...
    try:
            ...
            if data.get("type") == "EVENT":
                ...
                try:
                    updated_form = await FormService.process_event(
                        db=db, form_id=form_id, user_id=user.id, event_in=form_event
                    )
        ...
        if left_user:
            await manager.broadcast_to_form(
                str_form_id, {"type": "CURSOR_LEAVE", "userId": left_user.get("userId")}
            )
