...
async def require_owner(conn: asyncpg.Connection, organization_id: str, user_id: str):
...
async def add_member(
    ...
    user_row = await conn.fetchrow(
        "SELECT id FROM users WHERE email = $1", user_email.strip().lower()
    )
    if not user_row:
        raise HTTPException(
...
async def create_invite(
    conn: asyncpg.Connection, user_id: str, organization_id: str, email: str, role: str
) -> str:
    # Ensure owner
