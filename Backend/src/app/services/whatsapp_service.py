...
def set_session(phone_number: str, session_data: dict):
    ...
    redis_client.set(f"whatsapp:session:{phone_number}", json.dumps(session_data), ex=3600)
...
async def send_whatsapp_message(phone_number: str, message: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            print(f"Error sending WhatsApp message to {phone_number}: {e.response.status_code} - {e.response.text}")
...
async def handle_whatsapp_message(body: dict):
    ...
    if not sender_jid or not message_text:
        print(f"Received webhook with missing sender or message body: {json.dumps(body)}")
        return
    ...
    # --- Answer Validation ---
    is_valid, error_message, processed_answer = validate_answer(message_text, current_question)
